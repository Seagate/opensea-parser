...
{
    m_sDriveInfo.serialNumber.resize(SERIAL_NUMBER_LEN);
    strncpy((char*)m_sDriveInfo.serialNumber.c_str(), (char*)&pData[offset], SERIAL_NUMBER_LEN);
...
{
    m_sDriveInfo.firmware.resize(FIRMWARE_REV_LEN);
    strncpy((char*)m_sDriveInfo.firmware.c_str(), (char*)&pData[offset], FIRMWARE_REV_LEN);
...
{
    m_sDriveInfo.modelNumber.resize(MODEL_NUMBER_LEN);
    strncpy((char*)m_sDriveInfo.modelNumber.c_str(), (char*)&pData[offset], MODEL_NUMBER_LEN);
...
{
    uint16_t *identWordPtr = (uint16_t *)pData; // move it to the second page / sector
    ...
    uint64_t ieeeOUI = (M_BytesTo8ByteValue(0, 0, 0, 0, identWordPtr[217] & 0x0F, identWordPtr[216], identWordPtr[219], identWordPtr[218] & 0xF0)) >> 4;
    uint64_t uniqueID = M_BytesTo8ByteValue(0, 0, 0, identWordPtr[218] & 0x0F, identWordPtr[221], identWordPtr[220], identWordPtr[223], identWordPtr[222]);
...
{    
    ...
    create_Model_Number(0x36);
    create_Serial_Number( 32);
    ...
    {
        m_sDriveInfo.sTrans.transportType = ((identWordPtr[222] & 0xF000) >> 12);
    ...
    if (m_sDriveInfo.IDCapabilities & BIT26) //bit 27
    {
...
eReturnValues CAta_Identify_log::print_Identify_Information(JSONNODE *masterData)
{
    ...
    //Transport
    if (true)//(m_sDriveInfo.trans.transportReported)
    {
...
CAta_Identify_Log_30::CAta_Identify_Log_30( const std::string & fileName)
    :CLog(fileName)
    , m_name("ATA Identify Log Page 30")
    , m_status(IN_PROGRESS)
{
    if (get_Log_Status() == SUCCESS)
    {
        //pData = (uint8_t *) m_bufferData;
...
int CAta_Identify_Log_30::get_Interface_Type(uint8_t *pData)
{
    std::string interfaceType = "unknow";
    interfaceType.resize(BASIC);
    uint64_t interfaceOffset = 0x180;
