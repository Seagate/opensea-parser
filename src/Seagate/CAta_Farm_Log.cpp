...
eAnalyzeStatus CATA_Farm_Log::parse_and_Analyze_Farm_Log()
{
    ...
    if ((vFarmFrame[0].reliPage.heliumPresureTrip & 0x00FFFFFFFFFFFFFFLL) == 1)
    {
        anzStatus = PRESSURE;
    }
    else if ((vFarmFrame[0].errorPage.totalFlashLED & 0x00FFFFFFFFFFFFFFLL) >= 4)
    {
        anzStatus = FW_ASSERTION;
    }
    else if (((vFarmFrame[0].errorPage.attrIOEDCErrors & 0x00FFFFFFFFFFFFFFLL) >= 1) &&
            (vFarmFrame[0].errorPage.attrIOEDCErrors & 0xC000000000000000LL) == 0xC000000000000000LL)
    {
        anzStatus = DATA_PATH;
    }
    else if ((vFarmFrame[0].reliPage.attrSeekErrorRateNormal & 0x00FFFFFFFFFFFFFFLL) < 45)
    {
        anzStatus = SEEK_HARD_ERROR;
    }
    else
    {
        if ((vFarmFrame[0].errorPage.totalReadRecoveryAttepts & 0x00FFFFFFFFFFFFFFLL) < 1)
        {
            if ((vFarmFrame[0].errorPage.totalReallocations & 0x00FFFFFFFFFFFFFFLL) < 8)
            {
                anzStatus = NTF;
            }
            else
            {
                uint64_t maxObserverErr = vFarmFrame[0].reliPage.velocityObserver[0];

                for (uint32_t i = 1; i < 24; i++)
                {
                    if (maxObserverErr < vFarmFrame[0].reliPage.velocityObserver[i])
                    {
                        maxObserverErr = vFarmFrame[0].reliPage.velocityObserver[i];
                    }
                }

                if (((maxObserverErr & 0x00FFFFFFFFFFFFFFLL) >= 9675) &&
                    ((maxObserverErr & 0xC000000000000000LL) == 0xC000000000000000LL))
                {
                    anzStatus = UNRECOVERABLE_SERVO;
                }
                else
                {
                    anzStatus = UNRECOVERABLE_WRITE;
                }
        ...
        { 
            if (((vFarmFrame[0].errorPage.reallocatedSectors[14] & 0x00FFFFFFFFFFFFFFLL) < 8) &&
                ((vFarmFrame[0].errorPage.reallocatedSectors[14] & 0xC000000000000000LL) == 0xC000000000000000LL))
            { 
                anzStatus = UNRECOVERABLE_READ_GLIST;
            }
            else
            {
                anzStatus = UNRECOVERABLE_WRITE;
