...
void opensea_parser::prePython_unknown_params(JSONNODE* masterData, uint64_t value, uint16_t logPage, uint8_t subPage, uint16_t paramCode, uint32_t offset)
{
    if (g_parseUnknown)
    {
        ...
        temp << "scsi-log-page:0x" << std::hex << std::nouppercase << logPage << "," << std::hex << std::nouppercase << static_cast<uint16_t>(subPage) << ":0x" << std::hex << std::nouppercase << paramCode << ":" << std::dec << offset;
