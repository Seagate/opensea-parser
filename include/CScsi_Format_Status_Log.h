...
#pragma pack(pop)
    ...
    protected:
        ...
        uint8_t                     *m_FormatDataOutParamValue; //<! format Data out parameter value

        void get_Format_Parameter_Code_Description(std::string *valueData);
        void process_Format_Status_Data(JSONNODE *formatData);
        void process_Format_Status_Data_Variable_Length(JSONNODE *formatData);
        eReturnValues get_Format_Status_Data(JSONNODE *masterData);
    public:
        CScsiFormatStatusLog();
        explicit CScsiFormatStatusLog(uint8_t * buffer, size_t bufferSize, uint16_t pageLength);
